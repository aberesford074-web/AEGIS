...
function renderSalesAgent(data = {}) {
  ...
  metricCards('#sales-agent-summary', [['Prospects with phones', prospects.length], ['Briefs ready', ready], ['Approved outreach', approved], ['Phone provider', data.readiness?.telephony ? data.readiness.telephony : 'Not connected']]);
  $('#sales-agent-list').classList.toggle('empty', !prospects.length);
  $('#sales-agent-list').innerHTML = !prospects.length ? 'No prospects with phone numbers yet. Import a prospect list to begin.' : `<table><thead><tr><th>Prospect</th><th>Website</th><th>Phone</th><th>Latest brief</th><th></th></tr></thead><tbody>${prospects.map((prospect) => { const run = latestByProspect.get(prospect.id); const action = run?.status === 'ready' ? `<button class="ghost approve-sales-agent" data-id="${html(run.id)}">Approve brief</button>` : run?.status === 'approved' ? '<span class="channel">Approved</span>' : `<button class="ghost prepare-sales-agent" data-id="${html(prospect.id)}">Prepare brief</button>`; const websiteHref = safeExternalHref(prospect.website); return `<tr><td><strong>${html(prospect.company)}</strong><br><small>${html(prospect.contact_name || 'No named contact')}</small></td><td>${websiteHref ? `<a href="${websiteHref}" target="_blank" rel="noreferrer">${html(prospect.website)}</a>` : (prospect.website ? 'Invalid URL' : 'Not supplied')}</td><td>${html(prospect.phone)}</td><td>${run ? tag(run.status) : 'Not prepared'}</td><td>${action}</td></tr>`; }).join('')}</tbody></table>`;
